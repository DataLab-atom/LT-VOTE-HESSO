...
class HESSO(Optimizer):
    def __init__(self, params, variant='sgd', lr=required, first_momentum=None, second_momentum=None, \
                 ...
                 group_divisible=1, fixed_zero_groups=True, importance_score_criteria='default',args=None):
        ...
        if importance_score_criteria == 'default':
            ...
            self.importance_score_criteria_data = torch.ones(5,100).float()
            self.importance_score_criteria_masked = torch.ones(5).float()/5
            self.importance_score_criteria_cls_num_list = torch.tensor(args.cls_num_list).view(100,1).float()
        elif importance_score_criteria == 'LT-vote':
            self.cls_acc_list = torch.tensor([0 for i in range(args.num_class)])
            self.importance_score_criteria_cls_num_list = torch.tensor(args.cls_num_list).view(100,1).float()
            self.importance_score_criteria_data = torch.ones(5,100).float()
            self.importance_score_criteria_masked = torch.ones(5).float()/5
            self.up_date_importance_score_criteria()
        else:
    ...
    def up_date_importance_score_criteria(self, cls_acc_list=None):
        dot = self.importance_score_criteria_data@self.importance_score_criteria_cls_num_list
        self.importance_score_criteria_masked = torch.softmax(dot,dim=-1)
        if cls_acc_list!=None:
            print(self.num_steps,self.start_pruning_step)
            if self.num_steps >= self.start_pruning_step:
                masked_index = self.importance_score_criteria_masked.argmin(-1)[0]
                indexes = torch.ones(5).long()
                indexes[masked_index] = 0
                for i in torch.nonzero(indexes):
                    self.importance_score_criteria_data[i][(cls_acc_list>self.cls_acc_list).view(1,-1)] += 1
                dot = self.importance_score_criteria_data@self.importance_score_criteria_cls_num_list
                self.importance_score_criteria_masked = torch.softmax(dot,dim=-1)
                masked_index = torch.multinomial(torch.softmax(1-self.importance_score_criteria_masked,dim=-1), 1, replacement=True)[0] 
            else:
                masked_index =  np.random.randint(0,5)
            self.cls_acc_list = cls_acc_list
        else:
            masked_index =  np.random.randint(0,5)
        
        indexes = torch.ones(5).long()
        indexes[masked_index] = 0
        self.importance_score_criteria_masked[indexes] += self.importance_score_criteria_masked[masked_index]/4
        self.importance_score_criteria_masked[masked_index] = 0
        self.importance_score_criteria= {k:v.cuda() for k,v in zip(['magnitude','avg_magnitude','cosine_similarity','taylor_first_order','taylor_second_order'],self.importance_score_criteria_masked)}
