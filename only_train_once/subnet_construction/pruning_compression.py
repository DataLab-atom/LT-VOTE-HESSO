...
def automated_pruning_compression(oto_graph, model, merge_lora_to_base, unmerge_lora_to_base, export_huggingface_format, export_float16, \
    ...
    compressed_in_dim_pruned_idxes = {}
        ...
        for node in node_group.nodes.values():
            ...
            if not node.pruned_status['in_dim']:
                for name in node.param_names:
                    compressed_in_dim_pruned_idxes[name] = in_dim_pruned_idxes
    ...
    if export_huggingface_format:
        ...
        import numpy as np
        np.save(compressed_model_dir+'compressed_in_dim_pruned_idxes.npy',np.array(compressed_in_dim_pruned_idxes))
        print(compressed_model_dir+'compressed_in_dim_pruned_idxes.npy')
    elif ckpt_format == 'onnx':
        torch.onnx.export(
            model,
            oto_graph.dummy_input,
