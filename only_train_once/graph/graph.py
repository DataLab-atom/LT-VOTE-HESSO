...
class Graph:
    ...
    def build(self, model, dummy_input):
        ...
        for torch_node in trace_graph.nodes():
            node_ID = self.torch_node_id(torch_node)
            # Get Operation
            ...
            node = Node(id=node_ID, op_name=op_name, op=op, inputs=inputs, outputs=outputs, param_names=param_names, output_shape=output_shape)
    ...
    def _get_trace_graph(self, model, dummy_input, optimized_onnx=False):
        ...
        with torch.no_grad():
            ...
            # 手动禁用常量折叠
            def disable_constant_folding(graph):
                for node in graph.nodes():
                    if node.kind() == "onnx::Slice":
                        # 设置属性以禁用常量折叠
                        node.addAttribute("disable_constant_folding", 1)

            # 应用禁用常量折叠的函数
            disable_constant_folding(trace_graph)
