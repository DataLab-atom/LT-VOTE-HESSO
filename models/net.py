...
from models.resnet_gate import resnet32_rep_gete
...
def get_rep_layer(args):
    if args.dataset == 'cifar100':
        if args.ATO:
            return resnet32_rep_gete(args.num_class, use_norm= False).cuda()
        else:
...
class Net(nn.Module):
    def __init__(self,args, use_norm=False):
        ...
        for t in args.tasks:
            if t in self.unnorm_task:
                continue
            self.decoders[t] = get_liner(args, self.num_class,t, use_norm = (t in ['ldam_drw'] ) ).cuda()
    ...
    def forward(self, x ,output_type = 'feat',crt = False):
        ...
        self.shallow_outs = shallow_outs
        if self.shike:
            out_put['shike'] = self.shike_decoder(shallow_outs,crt)#[30* self.shike_decoder[i](x,output_type) for i in range(3)]
            
        return out_put
    
    def forward_again(self, x ,output_type = 'feat',crt = False):
        out_put = {}
        if isinstance(x,torch.Tensor):
            x = x.cuda(non_blocking=True)
            x,shallow_outs = self.encoder.forward_again(x)
            ...
            for i in range(3):
                x[i] = x[i].cuda(non_blocking=True)
                x[i],shallow_outs[i] = self.encoder.forward_again(x[i])
