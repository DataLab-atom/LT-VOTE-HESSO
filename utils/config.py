...
def parse_args(run_type = 'terminal'):
    ...
    # Checkpoints
    parser.add_argument('--out', default='./logs/results/', help='Directory to output the result')
    ...
    parser.add_argument("--mgda_mode",                     type = str,default='none')#choices=['l2', 'loss', 'loss+',
    parser.add_argument("--base_beta",  type = float ,default = 0.5)#choices=['l2', 'loss', 'loss+',
    parser.add_argument('--llm',action='store_true',default=False)
    parser.add_argument('--llm_name',type= str,default='alibaba')
    parser.add_argument('--llm_mode',type= int,default=0)
    
    # cat out
    parser.add_argument('--out_cut',action='store_true',default=True)
    ...
    parser.add_argument('--hesso',action='store_true',default=False)#'LT-vote'   default
    parser.add_argument('--isc', default='default', type=str, help="importance_score_criteria")#'LT-vote'   default
    parser.add_argument('--target_group_sparsity', default=0.7, type=float, help="target_group_sparsity")
    parser.add_argument('--save_all_epoch',action='store_true',default=False)
    parser.add_argument('--ATO',action='store_true',default=False)
    
    if run_type == 'terminal':
        args = parser.parse_args()
        args.loss_fn,args.tasks =  lossfun(args)
        args.out = f'{args.out}{args.dataset}/{args.loss_fn}@N_{args.num_max}_ir_{args.imb_ratio}/'+'hesso_{}_target_group_sparsity_{}/'.format(args.isc if args.hesso else ('ATO' if args.ATO else 'none'),args.target_group_sparsity) 
