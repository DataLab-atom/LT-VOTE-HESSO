...
from aug.transforms_large import *
from aug.randaug import *

import torchvision.transforms as transforms
from datasets.lt_data_tools import get_supper_targets

class LT_Dataset(Dataset):
    def __init__(self, root, txt, args, dataset, use_randaug=False,
                 split='train', aug_prob = 0.5, upgrade=1, downgrade=1):
        self.img_path = []
        self.labels = []
        
        if split=='train':
            transform = get_transform(dataset, args, split='train')
            self.transform = transform
            self.split = 'train'
        else:
            transform = get_transform(dataset, args, split='valid')
            self.transform = transform
            self.split = 'valid'
        ...
        self.aug_prob = aug_prob
        self.supper_targets =  get_supper_targets(self.targets,args.supper_classes,num_class=args.num_class)
        self.SP_cls = [(self.supper_targets == i).sum() for i in range(args.supper_classes)]
    ...
    def get_item(self, index, state, train=True):
        ...
        sp_targets = self.supper_targets[index]
        ...
        if train:
            if len(self.transform) == 1:
                ...
                return img, label, index,sp_targets
                ...
                return (img1, img2), label, index,sp_targets
                ...
                return (img1, img2, img3), label, index,sp_targets
    ...
    def __getitem__(self, index):
        if self.split == 'train':
            ...
            img, target, index,sp_targets = self.get_item(index, state, train=True)
        else:
            img, target, index,sp_targets = self.get_item(index, None, train=False)
        return img, target, index,sp_targets
...
class test_loader(Dataset):
    ...
    def __getitem__(self,idx):
        data, label, _,sp_label = self.dataset.get_item(self.indices[idx], self.state[idx], train=False)
        return data, label, self.indices[idx], self.state[idx],sp_label
