...
class Weight_acc:
    ...
    def update(self,logits,targets):
        ...
        for i in range(self.num_class):
            ...
            for t in self.tasks:
                if t == self.max_weight_task[i]:
                    continue
                
                if self.weigh[t][i] > self.weigh[self.max_weight_task[i]][i]:
    ...
    def cat_targets(self,logits,targets,epoch):
        ...
        self.weigh_save_list['conflict'][epoch][1] += torch.nonzero(torch.logical_not(same_index)).sum().item()
        # in conflict      
