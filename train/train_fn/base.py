...
def update_score_base(loader, tmp_loader, model,optimizer, n_samples_per_class, posthoc_la, num_test, accept_rate, tasks,args):
    model.eval()
    ...
    with torch.no_grad():
        ...
        if 'cifr' in args.dataset:
            ...
            tmp_loader = torch.utils.data.DataLoader(tmp_dataset, batch_size=128, shuffle=False, num_workers=8,pin_memory=True)
        


        isc_logits,isc_targets = [],[]
    ...
    #print(loader.dataset.curr_state.shape)
    ...
    if args.isc=='LT-vote':
        isc_logits = torch.tensor(isc_logits)
        isc_targets = torch.tensor(isc_targets)
        optimizer.up_date_importance_score_criteria(torch.tensor([(isc_logits[isc_targets == target] == target).sum() for target in range(args.num_class)]).long())
    else:
        optimizer.up_date_importance_score_criteria()
