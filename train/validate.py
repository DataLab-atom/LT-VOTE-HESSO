...
def valid_normal(args, valloader, model, criterion, epoch, per_class_num, num_class=10, mode='Test Stats', trainloader = None, tasks = None):
    ...
    with torch.no_grad():
        ...
        for data_tuple in valloader:
            ...
            # plot progress
            info_header = '({batch}/{size}) Total: {total:} | ETA: {eta:} | '.format(
                batch=batch_idx,
                size=len(valloader),
                #data=data_time.avg,
                #bt=batch_time.avg,
            ...
            for t in tasks:
                for index,info_name in enumerate([losses]):
    ...
    with torch.no_grad():
        for all_outputs,targets in zip(outputs_,targets_):
            prec1 = {}
            prec5 = {}
            for index,t in enumerate(temp_tasks):
                prec1[t], prec5[t] = accuracy(all_outputs[t], targets, topk=(1,5)) 
