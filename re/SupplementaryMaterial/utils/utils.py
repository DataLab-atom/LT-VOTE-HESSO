...
def transform_selection(cfg, mode):
    ...
    elif cfg['dataset']['dataset_name'] == 'Places-LT':
        return data_transforms_places[mode]
    elif cfg['dataset']['dataset_name'] == 'ImageNet-LT':
        return data_transforms_imagenet[mode]
    else :
        return data_transforms_cifar[mode]
...
def pre_compute_class_ratio(cfg, data_source):
    ...
    weights = np.log(max_num / np.array(ratios) + 0.01) / cfg['train']['div']   
...
def create_logger(cfg, rank=0, test=False):
    ...
    fhandler = logging.FileHandler(log_file, mode='w', encoding="utf8")
    fhandler.setFormatter(header)
...
def get_optimizer(cfg, model):
    ...
    cls_params = list(map(id, model.module.classifier.parameters()))
