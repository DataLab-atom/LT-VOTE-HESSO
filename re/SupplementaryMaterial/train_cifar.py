...
def train_sample(epoch, train_loader, model, optimizer, logger, class_weights):
    ...
    for step, (x, y, _) in enumerate(train_loader):
        ...
        if step % cfg['print_inteval'] == 0:
            print(('Trainnig Loss:{train_loss:.3f}, Ori Loss:{ori_loss:.3f}, Training Acc:{train_acc:.2f}').format(train_loss = loss.item(), ori_loss = loss.item(), train_acc = acc))
            logger.info(('Trainnig Loss:{train_loss:.3f}, Ori Loss:{ori_loss:.3f}, Training Acc:{train_acc:.2f}').format(train_loss = loss.item(), ori_loss = loss.item(), train_acc = acc))
...
if __name__ == '__main__':
	...
	# ----- LOAD PARAM -----
    path = "./configs/Cifar100_100.json" 
    ...
    for epoch in range(cfg['train']['max_epoch']):
        ...
        outline.append([h_acc, m_acc, t_acc, acc])
        ...
        np.save('outline.npy',np.array(outline))
