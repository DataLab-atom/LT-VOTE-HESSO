...
from .lt_data_tools import get_supper_targets
...
class CIFAR100_train(torchvision.datasets.CIFAR100):
    ...
    def get_item(self, index, state, train=True):
        img, target,sp_target = self.data[index], self.targets[index],self.supper_targets[index] 
        ...
        if train:
            if len(self.transform_train) == 1:
                ...
                return img, target, index,sp_target
                ...
                return (img1, img2), target, index,sp_target
                ...
                return (img1, img2, img3), target, index,sp_target
    ...
    def __getitem__(self, index):
        ...
        img, target, index,sp_target = self.get_item(index, state, train=True)
        return img, target, index,sp_target
...
class CIFAR100_val(torchvision.datasets.CIFAR100):
    def __init__(self, root, args,transform=None, indexs=None,
        ...
        self.supper_targets =  get_supper_targets(self.targets,args.supper_classes,args.num_class)
        self.SP_cls = [(self.supper_targets == i).sum() for i in range(args.supper_classes)]
